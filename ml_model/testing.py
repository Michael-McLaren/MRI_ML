...
#python ml_model/testing.py --num_epochs 100 --experiment_name trained_models/real
...
def main():
    ...
    val_data_loader = uterus_train_data.return_dataloader(batch_train, shuffle = False)
    ...
    mri_experiment.load_model(model_path, 'train_model', 99)
    
    
    x = np.load('trained_models/data/test_x.npy')
    y = np.load('trained_models/data/test_y.npy')
    
    train_ind = int(x.shape[0]*0.6)
    val_ind = int(x.shape[0]*0.8)
    ...
    j = 63
    x_norm = uterus_train_data.normalise(x_test)
    mri_experiment.example_fit(x_test[j], y_test[j], x_norm[j])
