...
def TwoCUMfittingConc(t, AIF, uptake, toff, plot=0):
    ...
    bnds=((0,1.2),(0.00001,10)) # Set upper and lower bounds for parameters
    resultsmatrix=np.zeros((len(vpmatrix),6))  # Initialise results array

    for i in range (0,len(vpmatrix)):
        Result=scipy.optimize.minimize(objfunConc,startguess,args=(np.array([vpmatrix[i]]),t,AIFnew,uptake),bounds=bnds, method='Nelder-Mead',options={'ftol':1e-14,'disp':False,'maxiter':300})
